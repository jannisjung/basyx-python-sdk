...
class TestAasClient(unittest.TestCase):
    ...
    @patch('basyx_client.aas.AasClient.get_shell')
    def test_get_submodel_references(self, mock_get_shell):
        # Create a mock AAS with empty submodel references
        mock_shell = Mock()
        mock_shell.submodel = []
        ...
        # Verify the get_shell method was called with correct arguments
        mock_get_shell.assert_called_once_with(shell_id)
    ...
    def test_get_submodels(self, mock_get_submodel, mock_get_submodel_refs):
        ...
        mock_ref = Mock(spec=model.ModelReference)
